...
(function (document, window) {
    window.svgFallback = {
        ...
        init: function init() {
            var attr = arguments.length <= 0 || arguments[0] === undefined ? 'data-png' : arguments[0];
        ...
        replaceImgSrc: function replaceImgSrc(attr) {
            if (!this.svgSupport) {
                ...
                while (imgLength--) {
                    var newSrc = images[imgLength].getAttribute(attr);
                    if (newSrc !== null) {
        ...
        addClass: function addClass() {
            var className = this.svgSupport ? ' svg' : ' no-svg';
